...
export async function executeAnthropic(request: AIRequest): Promise<AIResponse> {
  ...
  const initialUserMessages: Record<string, unknown>[] = [
    ...
    ...chunks.map((chunk, i) => ({
      role: 'user' as const,
      // Cache control goes on content blocks, not the message itself
      content: i === cacheBreakpointIndex
        ? [{ type: 'text' as const, text: chunk.content, cache_control: { type: 'ephemeral' as const } }]
        : chunk.content,
    })),
